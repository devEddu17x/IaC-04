resource "aws_security_group_rule" "sqs_inbound_sign_lambda" {
  ...
  to_port                  = 443
  protocol                 = "tcp"
  security_group_id        = aws_security_group.sg_sqs_vpce.id
  source_security_group_id = aws_security_group.sg_sign_lambda.id
}
...
resource "aws_security_group_rule" "sqs_inbound_crop_lambda" {
  ...
  protocol                 = "tcp"
  security_group_id        = aws_security_group.sg_sqs_vpce.id
  source_security_group_id = aws_security_group.sg_crop_lambda.id
}
...
resource "aws_security_group_rule" "sign_lambda_outbound" {
  ...
  to_port                  = 443
  protocol                 = "tcp"
  security_group_id        = aws_security_group.sg_sign_lambda.id
